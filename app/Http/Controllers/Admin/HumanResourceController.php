...
class HumanResourceController extends Controller
{
    ...
    {
        return Excel::download(new HumanResourceExport($request->all()), 'human_resources.xlsx');
    }
    ...
    //     $pdf = Pdf::loadView('admin.humanresouce.pdfExport', ['data' => $result])
    //         ->setPaper([0, 0, 1400, 2000], 'landscape');

    //     return $pdf->download('Human_Resource_Report.pdf');
    // }
    ...
    public function exportPdf(Request $request)
    {
       ...
       // 🔹 After filters applied
        ...
        $chunkSize = 50; // process 10 records per chunk for performance
        $tempFiles = [];
        ...
        foreach ($chunks as $index => $chunk) {
            ...
            foreach ($chunk as $row) {
                $step4 = $row->hrSteps->where('step_number', 4)->where('file_type', 'photo')->first();
                $step6 = $row->hrSteps->firstWhere('step_number', 6);
                $cnic_front = $row->hrSteps->where('step_number', 3)->where('file_type', 'cnic front')->whereNotNull('file_name')->first();
                $cnic_back = $row->hrSteps->where('step_number', 3)->where('file_type', 'cnic back')->whereNotNull('file_name')->first();
                $passport_front = $row->hrSteps->where('step_number', 2)->where('file_type', 'passport front')->whereNotNull('file_name')->first();
                $passport_back = $row->hrSteps->where('step_number', 2)->where('file_type', 'passport back')->whereNotNull('file_name')->first();
                $passport_third_image = $row->hrSteps->where('step_number', 2)->where('file_type', 'passport third image')->whereNotNull('file_name')->first();
                ...
                $passportPhotoPath = $step4 ? $step4->file_name : null;
                $passportPhotoBase64 = $passportPhotoPath && file_exists($passportPhotoPath)
                    ? 'data:image/jpeg;base64,' . base64_encode(file_get_contents($passportPhotoPath))
                    : null;
                ...
                $result[] = [
                    ...
                    'passport_photo' => $passportPhotoPath,
                    'passport_photo_base64' => $passportPhotoBase64,
                    'visa_type' => $step6->visa_type ?? '',
                    'visa_status' => $visa_status,
                    'visa_issue_date' => $step6->visa_issue_date ?? '',
                    'visa_expiry_date' => $step6->visa_expiry_date ?? '',
                    'flight_date' => $step6->flight_date ?? '',
                    'flight_route' => $step6->flight_route ?? '',
                    'cnic_taken_status' => ($cnic_front || $cnic_back) ? 'Taken' : 'Not Taken',
                    'passport_taken_status' => ($passport_front || $passport_back || $passport_third_image) ? 'Taken' : 'Not Taken',
                    'id' => $row->id,
                    'name' => $row->name ?? '',
            ...
            // 🔹 Render lightweight PDFs
            $pdf = Pdf::loadView('admin.humanresouce.pdfExport', ['data' => $result])
                ->setPaper('a4', 'landscape');

            $filePath = $tempPath . '/chunk_' . $index . '.pdf';
            $pdf->save($filePath);
            $tempFiles[] = $filePath;
        }
        try{
